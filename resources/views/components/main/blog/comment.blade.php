    ...
    <article class="blog-article-comment" id="{{ $comment->presenter()->elementId() }}">
        <section class="blog-article-comment-left">
            <img src="{{ $comment->avatar_url }}"
                alt="{{ __('Avatar for: :name', ['name' => $comment->commenter['display_name']]) }}"
                class="blog-article-comment-avatar" />
        </section>
        <section class="blog-article-comment-right">
            <section class="blog-article-comment-header">
                ...
                <div class="blog-article-comment-header-right">
                    <small
                        title="{{ $comment->post->created_at }}">{{ __('Posted :duration', ['duration' => $comment->post->created_at->longRelativeToNowDiffForHumans()]) }}</small>
                </div>
            </section>
            ...
            <section class="blog-article-comment-actions">
                @can('reply', $comment)
                    ...
                    @else
                        <a
                            href="{{ sprintf('%s#comment-%d', route('blog.single', ['article' => $article, 'parent_comment_id' => $comment->getKey()]), $comment->getKey()) }}">
                            @svg('fas-reply', 'me-2')
                            {{ __('Reply') }}
                        </a>
                    @endisset
